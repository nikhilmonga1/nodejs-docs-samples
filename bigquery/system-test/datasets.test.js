...
describe(`bigquery:datasets`, function () {
  after(() => bigquery.dataset(datasetId).delete({ force: true }).catch(() => undefined));
  ...
  it(`should list datasets`, (done) => {
    ...
    setTimeout(() => {
      ...
      assert.notEqual(output.indexOf(`Datasets:`), -1);
      assert.notEqual(output.indexOf(datasetId), -1);
      done();
    }, 5000);
  });
  ...
  it(`should return the size of a dataset`, function () {
    ...
    assert.notEqual(output.indexOf(`Size of hacker_news`), -1);
    assert.notEqual(output.indexOf(`MB`), -1);
    output = run(`${cmd} size ${datasetId}`, cwd);
    assert.notEqual(output.indexOf(`Size of ${datasetId}: 0 MB`), -1);
  });
