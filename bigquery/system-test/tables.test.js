...
describe('bigquery:tables', () => {
  ...
  it(`should list tables`, (done) => {
    ...
    setTimeout(() => {
      ...
      assert.notEqual(output.indexOf(`Tables:`), -1);
      assert.notEqual(output.indexOf(tableId), -1);
      done();
    }, 5000);
  });
  ...
  it(`should import a local file`, () => {
    ...
    assert.notEqual(output.indexOf(`started.`), -1);
    assert.notEqual(output.indexOf(`completed.`), -1);
    return new Promise((resolve, reject) => {
      setTimeout(() => {
